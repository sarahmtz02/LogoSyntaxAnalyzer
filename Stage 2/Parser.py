...
class Parser:
	...
	def __init__(self, filepath):
		...
		"""
		FIRST SETS DE TOKENS AGREGADOS CON GITHUB COPILOT
		"""
		...
		self.firstExtendedEqualityExpression = set((Tag.NEQ, ord('=')))
		...
		self.firstMovementStatement = set((Tag.LEFT, Tag.RIGHT, Tag.FORWARD, Tag.BACKWARD, Tag.SETX, Tag.SETY, Tag.SETXY, Tag.HOME))
		...
		self.firstExpression = self.firstPrimaryExpression.union(set((ord('-'), ord('!'))))
		
		self.firstElement = set((Tag.STRING,)).union(self.firstExpression)

		self.firstElementList = set((ord(','),))
		self.firstIdentifierList = set((ord(','),))
		self.firstIdentifier = set((Tag.ID,))
		self.firstAssigmentStatement = set((Tag.ID,))
		self.firstDeclarationStatement = set((Tag.VAR,))
		self.firstTextStatement = set((Tag.PRINT,))
		self.firstDrawingStatement = self.firstDrawingStatement.union(set((Tag.CLEAR, Tag.CIRCLE, Tag.ARC, Tag.PENUP, Tag.PENDOWN, Tag.COLOR, Tag.PENWIDTH)))
		self.firstMovementStatement = self.firstMovementStatement.union(set((Tag.LEFT, Tag.RIGHT, Tag.FORWARD, Tag.BACKWARD, Tag.SETX, Tag.SETY, Tag.SETXY, Tag.HOME)))
		...
		self.firstAssigmentStatement = set((Tag.ID,))
		self.firstDeclarationStatement = set((Tag.VAR,))
		self.firstSimpleStatement = self.firstDeclarationStatement.union(self.firstAssigmentStatement).union(self.firstMovementStatement).union(self.firstDrawingStatement).union(self.firstTextStatement)
		self.firstStatement = self.firstSimpleStatement.union(self.firstStructuredStatement)
		self.firstStatementSequence = self.firstStatement
		self.firstProgram = self.firstStatement
		self.firstExpression = self.firstPrimaryExpression.union(set((ord('-'), ord('!'))))
	...
	def repetitiveStatement(self):
		if self.token.tag == Tag.WHILE:
			...
			self.error("expected a repetitive expression before " + str(self.token))
	...
	def structuredStatement(self):
		if self.token.tag in self.firstStructuredStatement:
			...
			self.error("expected a structured expression before " + str(self.token))
	...
	def movementStatement(self):
		if self.token.tag == Tag.LEFT:
			...
			self.rightStatement()
		elif self.token.tag == Tag.FORWARD:
			...
			self.error("Expected a movement command")
